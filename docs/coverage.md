...
| `impl:dec-0029` | done:E1-L6 | already built: `internal/perf` spawns the shipped binary and asserts the median, `internal/index` and `internal/ledger/fixture` gate on the minimum, `hooks/pre-commit` measures the spawn floor, the perf job runs both platforms on `budget.go` unmodified. The entry records the discipline those already implement |
| `trigger:dec-0029:2f2d89` | done | already the case: `internal/index/latency_test.go`'s cache-beats-files comparison reads the uncontended minimum, and its comment states why a comparative claim must |
| `trigger:dec-0029:dc5476` | deferred:trigger | revisit trigger: enough perf runs accumulate to separate the between-run populations, at which point a percentile ceiling could be derived rather than chosen. No watch mechanism exists; the step summary publishes the samples every run |
| `trigger:dec-0029:dcb91b` | deferred:trigger | revisit trigger: a widening tail with the minimum beside the median — dira getting slower rather than a noisy runner. Visible in the perf job's published sample list, asserted by nothing (dec-0026) |
| `impl:dec-0030` | done:E4-L1 | `internal/kazi` — Snapshot()/Status() decode against a pinned, committed fixture corpus (never a live kazi in any test); a typed `Unavailable`/`NotFound` on every failure path; `ContractDrift` rather than an error on a `schema_version` bump |
| `trigger:dec-0030:3c4ad4` | deferred:trigger | not work — a condition to watch (kazi registering `portfolio`). **No watch mechanism exists yet** (see gap list) |
| `impl:dec-0031` | covered:site | `docs/plan/website.md` W1-T1..W2-T8; `site/` scaffolded, deploy stays workflow_dispatch-only behind `pages-live` until the founder activation runbook (W2-T8) is run |
| `trigger:dec-0031:453af0` | deferred:trigger | revisit trigger: kazi's site stack is itself replaced or found to have a structural flaw dira would inherit. No watch mechanism exists yet (see gap list) |
| `trigger:dec-0032:636988` | deferred:trigger | revisit trigger: T-BUG1.1's stress test finds a genuine live create-vs-create collision in `ledger.Add`, at which point the fix must also touch `Add` itself, not only allocation. No watch mechanism exists yet (see gap list) |
| `trigger:dec-0033:15a86b` | deferred:trigger | revisit trigger: dec-0032 ships and id reuse via deletion stops being possible, at which point keeping deletion as the sole retirement path may be safe and this decision should be revisited. No watch mechanism exists yet (see gap list) |
